...
for optimal Sarsa_LAmbda (I think it's not totally guaranteed, sometimes it's one stepp/or two-step un-optimal?)
alpha = 0.20
eps = 0.5
maxiters = 15000 (but if all goes well then 10k maxiters should also converge)
lambda_factor = 0.65 (but also small lambda_factor 0.2 worked nicely)
...
## some useful starting parameters
lambda_factor = 0.2
...
## some useful datastructures for algorithms,
## NOTE! they are globals, and will be updated inside many functions
...
"""E_trace is maintained for all (s,a) pairs"""

E_trace = {}
for row in range(rows_count):
    for col in range(columns_count):
        for action in actions:
            E_trace[ ((row,col),action) ] = 0.0
    ...
    for c in range(columns_count):
        for act in actions:
            if not isTerminal(r, c):
                ...
                QDict[(
                (r, c), act)] = 0  ## for terminal (s,a) pairs we must initialize at zero according to Sutton & Barto
    ...
    if (r1 != r0 and c1 != c0):  ##diagonal moves are here
        ...
        if r0 - r1 == 1:
            if c1 - c0 == 1:
                ...
                raise Exception("walkedPath upDiagonal bug")
        ##down
        elif r1 - r0 == 1:
            if c1 - c0 == 1:
                symbol = downRight
                ...
                raise Exception("walkedPath downDiagonal bug")
        else:
            raise Exception("walkedPath diagonal bug")
...
"""update Etrace for all state-act pairs
parameters are (s,a) pair for curState and curAct
return void"""

"""def updateE_trace(visitedState, curAct):
    global E_trace, actions
    global rows_count, columns_count
    global gamma, lambda_factor

    r = visitedState[0]
    c = visitedState[1]
    tempVal = E_trace[ visitedState, curAct  ]
    tempVal += 1
    E_trace[visitedState, curAct] = tempVal
    ...
    for row in range(rows_count):
        for col in range(columns_count):
            for action in actions:
                decayedVal = E_trace[(row,col), action]
                decayedVal = decayedVal * gamma * lambda_factor
                E_trace[(row,col), action] = decayedVal"""





"""resets Etrace so it's zeroed out for all new episodes"""
def nullifyE_trace():
    global E_trace, actions
    global rows_count, columns_count
    E_trace = {}
...
print("hello SarsaLambda, WindyGridWorld!!!")

for iteration in range(1, maxiters + 1):
    print("SarsaLambda_WindyWorld iterCount == ", iteration, "eps == ", epsilon, end=" ")

    nullifyE_trace()

    """episode begins in earnest! now"""
    S = start_state  ## get startState and startAction (start_state was pre-defined already)
    ##A = getActionFromEpsGreedyPolicy(S)
    ##optA = refactored_argmaxActQ(S)  ## NOTE!!! important! we must explicitly get argmaxAct, because getActFromEpsGreedy is not guaranteed to return optAct!!!
    ...
    while not isTerminal(S[0], S[1]):
        ...
        delta_factor = reward + gamma * newValue - curValue

        ## update the currently visitedState,Act pair
        tempVal = E_trace[S, A]
        tempVal += 1
        E_trace[S, A] = tempVal

        ## update the decays for E_trace
        for r in range(rows_count):
            for c in range(columns_count):
                for a in actions:
                    tempSA = QDict[ (r,c), a ]
                    tempEtrace = E_trace[ (r,c), a ]
                    QDict[ (r,c), a ] = tempSA + alpha * delta_factor * tempEtrace
                    E_trace[ (r,c), a ] = gamma * lambda_factor *tempEtrace



        S = Sprime
        A = Aprime
        ##optAprime = refactored_argmaxActQ(Sprime)  ## NOTE!!! important! we must explicitly get argmaxAct, because getActFromEpsGreedy is not guaranteed to return optAct!!!
...
for SApair in QDict:
    if not isTerminal(SApair[0][0], SApair[0][1]) and (QDict[SApair] != 0):
        curState = SApair[0]
        optA = refactored_argmaxActQ(curState)
        r0 = curState[0]
        c0 = curState[1]
...
while curState != term_state:
    gotStuckCounter += 1
    bestAct = refactored_argmaxActQ(curState)
    newState, reward, wasNewTerminalState = makeWindyWorldAction(curState,
                                                                 bestAct)  ## using the good-old MakeWindyWorldAction function just for debug purposes!!!
    ## you dont want to change too many things at the same time!!!
    sym = getWalkedPathSymbol(curState,
                              newState)  ## testing the new fucntion, which should give us correct symbols for the path on emptyBoard
    ...
    if gotStuckCounter > 200:
        raise Exception(
            'optimalWalkPath across emptyBoard did not succeed (probably because Qfunc was not exactly optimal)')
